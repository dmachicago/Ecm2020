...
namespace EcmArchiveClcSetup
{
	public class clsValidateCodes
	{
		...
		public bool SystemParmsCodesExist(string SysParm, string InsertSql)
		{
			...
			if (iCnt == 0)
			{
				B = DB.ExecuteSqlNewConn(InsertSql);
			}
			return B;
		}
		...
		public void valAvailFileTypes()
		{
			...
			this.AvailFileTypesExist("*", InsertSql);
		...
		public void LoadProfile(string ProfileName, string ProfileDesc)
		{
			try
			{
				...
				if (iCnt == 0)
				{
					...
					bool B = DB.ExecuteSqlNewConn(InsertSql);
					if (! B)
					{
						LOG.WriteToArchiveLog((string) ("ERROR: LoadProfile 100a - " + "\r\n" + InsertSql));
					}
				}
			}
			catch (Exception ex)
			{
				LOG.WriteToArchiveLog((string) ("ERROR: LoadProfile 100 - " + ex.Message + "\r\n" + ProfileName));
			}
		...
		public void LoadProfileItems(string ProfileName, string SourceTypeCode)
		{
			try
			{
				...
				if (iCnt == 0)
				{
					...
					if (! B)
					{
						...
						if (! B)
						{
							LOG.WriteToArchiveLog((string) ("ERROR: LoadProfileItems 100a - " + "\r\n" + InsertSql));
						}
			...
			{
				LOG.WriteToArchiveLog((string) ("ERROR: LoadProfileItems 100 - " + ex.Message + "\r\n" + ProfileName + " : " + SourceTypeCode));
			}
		...
		public void SetDefaultUserParms(string UserGuid)
		{
			...
			int b = DB.ckUserStartUpParameter(UserGuid, "CONTENT WORKING DIRECTORY");
			if (b == 0)
			{
		...
		public void addDefaultAttributes(string AttributeName, string AttributeDataType, string AttributeDesc, string AssoApplication)
		{
			...
			if (! B)
			{
				...
				LOG.WriteToArchiveLog((string) ("ERROR 62952.14: Sql Statement " + "\r\n" + S));
			}
		...
		{
			clsRUNPARMS RP = new clsRUNPARMS();
