...
namespace EcmArchiver
{
    public class clsValidateCodes
    {
        ...
        public void valSystemParmsCodes()
        {
            string SysParm = "";
            string InsertSql = "";
        ...
        public bool SystemParmsCodesExist(string SysParm, string InsertSql)
        {
            bool B = false;
            int iCnt = 0;
            string S = "Select count(*) from SystemParms  where SysParm = '" + SysParm + "' ";
        ...
        public void valAvailFileTypes()
        {
            string InsertSql = "";
            InsertSql = "INSERT [AvailFileTypes] ([ExtCode]) VALUES ('*')";
            AvailFileTypesExist("*", InsertSql);
        ...
        public bool AvailFileTypesExist(string ExtCode, string InsertSql)
        {
            bool B = false;
            int iCnt = 0;
            string S = "Select count(*) from AvailFileTypes  where ExtCode = '" + ExtCode + "'";
            iCnt = DBARCH.iCount(S);
            if (iCnt == 0)
            {
        ...
        public bool AttachmentTypeExist(string AttachmentCode, string InsertSql)
        {
            bool B = false;
            int iCnt = 0;
            string S = "Select count(*) from AttachmentType where AttachmentCode = '" + AttachmentCode + "' ";
            iCnt = DBARCH.iCount(S);
            if (iCnt == 0)
            {
        ...
        public bool SourceTypeExist(string SourceTypeCode, string SourceTypeDesc, string Indexable)
        {
            // INSERT INTO [SourceType]([SourceTypeCode],[StoreExternal],[SourceTypeDesc],[Indexable])VALUES('XX',0,'XX',1)
            bool B = false;
            int iCnt = 0;
            string S = "Select COUNT(*) FROM [SourceType] where [SourceTypeCode] = '" + SourceTypeCode + "' ";
            iCnt = DBARCH.iCount(S);
            if (iCnt == 0)
            {
        ...
        public void LoadProfile(string ProfileName, string ProfileDesc)
        {
            try
            {
                string S = "Select COUNT(*) from [LoadProfile] where ProfileName = '" + ProfileName + "'";
                int iCnt = DBARCH.iCount(S);
                if (iCnt == 0)
                {
                    string InsertSql = "INSERT INTO LoadProfile (ProfileName,ProfileDesc) VALUES ('" + ProfileName + "','" + ProfileName + "')";
                    bool B = DBARCH.ExecuteSqlNewConn(90605, InsertSql);
                    if (!B)
                    {
                        LOG.WriteToArchiveLog("ERROR: LoadProfile 100a - " + Constants.vbCrLf + InsertSql);
                    }
                }
            }
            catch (Exception ex)
            {
                LOG.WriteToArchiveLog("ERROR: LoadProfile 100 - " + ex.Message + Constants.vbCrLf + ProfileName);
            }
        }

        public void LoadProfileItems(string ProfileName, string SourceTypeCode)
        {
            try
            {
                string S = "Select COUNT(*) FROM [LoadProfileItem] where [ProfileName] = '" + ProfileName + "' and SourceTypeCode = '" + SourceTypeCode + "'";
                int iCnt = DBARCH.iCount(S);
                if (iCnt == 0)
                {
                    string InsertSql = "INSERT INTO [LoadProfileItem] ([ProfileName] ,[SourceTypeCode]) VALUES ('" + ProfileName + "','" + SourceTypeCode + "')";
                    bool B = DBARCH.ExecuteSqlNewConn(90606, InsertSql);
                    if (!B)
                    {
                        ...
                        if (!B)
                        {
                            LOG.WriteToArchiveLog("ERROR: LoadProfileItems 100a - " + Constants.vbCrLf + InsertSql);
                        }
                    }
                }
            }
            ...
            {
                LOG.WriteToArchiveLog("ERROR: LoadProfileItems 100 - " + ex.Message + Constants.vbCrLf + ProfileName + " : " + SourceTypeCode);
            }
        }

        public void SetDefaultUserParms(string UserGuid)
        {
            var SI = new clsSAVEDITEMS();
            string SaveName = "UserStartUpParameters";
            string SaveTypeCode = "StartUpParm";
            string TempDir = System.IO.Path.GetTempPath();
            int b = DBARCH.ckUserStartUpParameter(UserGuid, "CONTENT WORKING DIRECTORY");
            if (b == 0)
            {
                SI.setSavename(ref SaveName);
                SI.setSavetypecode(ref SaveTypeCode);
                SI.setUserid(ref UserGuid);
            ...
            if (b == 0)
            {
                ...
                SI.setSavetypecode(ref SaveTypeCode);
                SI.setUserid(ref UserGuid);
                string argval2 = "RETENTION YEARS";
                SI.setValname(ref argval2);
                string argval3 = "10";
                SI.setValvalue(ref argval3);
                SI.Insert();
            }
            ...
            if (b == 0)
            {
                ...
                SI.setSavetypecode(ref SaveTypeCode);
                SI.setUserid(ref UserGuid);
                string argval4 = "DBARCH WARNING LEVEL";
                SI.setValname(ref argval4);
                string argval5 = "100";
                SI.setValvalue(ref argval5);
                SI.Insert();
            }
            ...
            if (b == 0)
            {
                ...
                SI.setSavetypecode(ref SaveTypeCode);
                SI.setUserid(ref UserGuid);
                string argval6 = "DBARCH RETURN INCREMENT";
                SI.setValname(ref argval6);
                string argval7 = "50";
                SI.setValvalue(ref argval7);
                SI.Insert();
            }
        ...
        public void addDefaultAttributes(string AttributeName, string AttributeDataType, string AttributeDesc, string AssoApplication)
        {
            ...
            if (!B)
            {
                ...
                LOG.WriteToArchiveLog("ERROR 62952.14: Sql Statement " + Constants.vbCrLf + S);
            }
        }

        public void AddUserSelectableParameters(string UserGuidID)
        {
        ...
        public void AddDflt(string sKey, string sValue, string sUser)
        {
            var RP = new clsRUNPARMS();
            int iCnt = RP.cnt_PKI8(sKey, sUser);
            if (iCnt == 0)
            {
                RP.setParm(ref sKey);
                RP.setParmvalue(ref sValue);
                RP.setUserid(ref sUser);
                RP.Insert();
            }
        ...
        public bool addHelpInfo(string HelpName, string HelpEmailAddr, string HelpPhone, string AreaOfFocus, string HoursAvail, int EmailNotification)
        {
            // INSERT INTO [SourceType]([SourceTypeCode],[StoreExternal],[SourceTypeDesc],[Indexable])VALUES('XX',0,'XX',1)
            bool B = false;
            int iCnt = 0;
