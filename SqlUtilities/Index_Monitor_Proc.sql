--exec UTIL_IndexUsage [Person],[Address]

SET ANSI_NULLS ON;
SET QUOTED_IDENTIFIER ON;
SET ANSI_PADDING ON;
GO
IF OBJECT_ID('dbo.UTIL_IndexUsage', 'P') IS NOT NULL
    BEGIN
        DROP PROCEDURE dbo.UTIL_IndexUsage;
END;
GO
CREATE PROCEDURE UTIL_IndexUsage @SchemaName SYSNAME    = '', 
                                 @TableName  SYSNAME    = '', 
                                 @Sort       TINYINT    = 5, 
                                 @Delimiter  VARCHAR(1) = ','
AS
...
/*******************************************************************************************************************
Reports index stats, index size+rows, member seek + include columns as two 
comma separated output columns, and index usage stats for one or more tables 
and/or schemas. Flexible parameterized sorting.
