...
-- UTIL_FindDB_BadPerformers.sql
/* TOP SLOW REQUESTS */
/* time is in microseconds */

SELECT top 15 creation_time, 
       last_execution_time, 
...
SELECT top 25 DB_NAME(st.dbid) DBName, 
...
ORDER BY s.total_elapsed_time desc , command;
...
WHERE o.type = 'u'
     ...
     OR -- Indexes that have been updated but not used
     (s.user_seeks = 0
      AND s.user_scans = 0
...
/* Monitor query plans top 50 worst plans by total_worker_time */
...
(
    SELECT qt.text
    FROM sys.dm_exec_sql_text(qs.sql_handle) AS qt FOR XML PATH('')
       ...
       qs.open_resultset_count, 
       qs.row_count, 
       qs.granted_query_memory, 
       qs.transaction_isolation_level
--,qs.*
...
ALTER INDEX IX_IndexName ON Table1 REBUILD WITH(ONLINE = ON);   
GO
-- W. Dale Miller
-- DMA, Limited
