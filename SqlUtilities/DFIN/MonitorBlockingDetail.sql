...
(
    ...
    CREATE TABLE DFINAnalytics.[dbo].[DFS_TranLocks]
    ([SPID]                 [INT] NOT NULL, 
...
go
-- exec sp_DFS_MonitorLocks
-- select top 20 * from [master].[dbo].[DFS_TranLocks] order by RowNbr desc;
...
begin
     ...
     INSERT INTO [master].[dbo].[DFS_TranLocks]
     ([SPID], 
      [DatabaseName], 
      [LockedObjectName], 
      [LockedObjectId], 
	 ...
	 update [master].[dbo].[DFS_TranLocks] set RunID = @RunID, CreateDate = getdate() where RunID is null
