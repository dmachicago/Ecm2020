...
SET @iTotalRecs = (SELECT
                          COUNT (*) 
                          FROM view_EDW_HealthInterestList);
SET @iCnt = (SELECT
                    COUNT (*) 
                    FROM view_EDW_HealthInterestList_CT);
PRINT 'Row count from view_EDW_HealthInterestList_CT : ' + CAST (@iTotalRecs AS nvarchar (50)) ;
PRINT 'Row count from parent view_EDW_HealthInterestList: ' + CAST (@iCnt AS nvarchar (50)) ;
...
--select * from view_EDW_HealthInterestList_CT WHERE NodeID = 85807;
