...
GO
-- drop table DIM_EDW_CombinedHAViews
...
BEGIN
    ...
    EXEC @iRows = proc_QuickRowCount 'DIM_EDW_CombinedHAViews';
    IF @iRows = 0
   AND EXISTS (SELECT
                      name
                      FROM sys.tables
    ...
    OR NOT EXISTS (SELECT
        ...
        BEGIN
            ...
            INTO
                 DIM_EDW_CombinedHAViews
                   FROM
            DBO.DIM_EDW_HFIT_HEALTHASSESMENTUSERRISKCATEGORY AS HARISKCATEGORY
                JOIN DBO.DIM_EDW_HFit_HealthAssesmentUserRiskArea AS HAUSERRISKAREA
                    ON
    ...
    DELETE FROM		DIM_EDW_CombinedHAViews	  -- DIM_EDW_HFIT_HealthAssesmentUserAnswers
    WHERE
          USERANSWERITEMID IN ( SELECT
                                       CT_HealthAssesmentUserAnswers.ItemID
                                       FROM CHANGETABLE (CHANGES HFIT_HealthAssesmentUserAnswers, NULL) AS CT_HealthAssesmentUserAnswers
                                       WHERE
           ...
           SET
             ...
             ,S.LastModifiedDate = T.LastModifiedDate
               FROM HFIT_HealthAssesmentUserAnswers AS T
                        JOIN DIM_EDW_CombinedHAViews AS S
                            ON
                            S.USERANSWERITEMID = T.ItemID
                        AND S.SVR = T.SVR
                        AND S.DBNAME = T.DBNAME
                        JOIN CHANGETABLE ( CHANGES HFIT_HealthAssesmentUserAnswers , NULL) AS CT_HealthAssesmentUserAnswers
                            ON
    ...
    DELETE FROM DIM_EDW_CombinedHAViews
    WHERE
          GroupResultsItemID IN ( SELECT
                                         CT_HFit_HealthAssesmentUserQuestionGroupResults.ItemID
                                         FROM CHANGETABLE (CHANGES FACT_HFit_HealthAssesmentUserQuestionGroupResults, NULL) AS CT_HFit_HealthAssesmentUserQuestionGroupResults
                                         WHERE
                                         SYS_CHANGE_OPERATION = 'D') ;
           ...
           SET
             ...
             ,DeletedFlg = 0
               FROM FACT_HFit_HealthAssesmentUserQuestionGroupResults AS T
                        ...
                        AND S.SVR = T.SVR
                        AND S.DBNAME = T.DBNAME
                        JOIN CHANGETABLE ( CHANGES FACT_HFit_HealthAssesmentUserQuestionGroupResults , NULL) AS CT_HFit_HealthAssesmentUserQuestionGroupResults
                            ON
    ...
    WHERE
          USERQUESTIONITEMID IN ( SELECT
                                         ...
                                         FROM CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERQUESTION, NULL) AS CT_CMS_HFIT_HEALTHASSESMENTUSERQUESTION
                                         WHERE
                                         SYS_CHANGE_OPERATION = 'D') ;
               ...
               SET
                 ...
                 ,S.LastModifiedDate = T.LastModifiedDate
                   ...
                   FROM FACT_HFIT_HEALTHASSESMENTUSERQUESTION AS T
                            ...
                            AND S.SVR = T.SVR
                            AND S.DBNAME = T.DBNAME
                            JOIN CHANGETABLE ( CHANGES FACT_HFIT_HEALTHASSESMENTUSERQUESTION , NULL) AS CT_HAQ
                                ON
    ...
    WHERE
          USERRISKAREAITEMID IN ( SELECT
                                         ...
                                         FROM CHANGETABLE (CHANGES HFit_HealthAssesmentUserRiskArea, NULL) AS CT_HFit_HealthAssesmentUserRiskArea
                                         WHERE
                                         SYS_CHANGE_OPERATION = 'D') ;
           ...
           SET
             ...
             ,DeletedFlg = 0
               FROM FACT_HFit_HealthAssesmentUserRiskArea AS T
                        ...
                        AND S.SVR = T.SVR
                        AND S.DBNAME = T.DBNAME
                        JOIN CHANGETABLE ( CHANGES HFit_HealthAssesmentUserRiskArea , NULL) AS CT_HFit_HealthAssesmentUserRiskArea
                            ON
