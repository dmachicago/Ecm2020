...
GO
PRINT 'Executing view_EDW_Eligibility_MART.sql';
GO
...
IF EXISTS (SELECT
           ...
           FROM sys.views
           WHERE
                  name = 'view_EDW_Eligibility_MART') 
    BEGIN
        DROP VIEW
             view_EDW_Eligibility_MART;
    END;
...
FROM
     BASE_CMS_Role AS ROLES
          ...
          ON
       ROLES.RoleID = MemberROLE.RoleID
	  and ROLES.DBNAME = MemberROLE.DBNAME
          JOIN BASE_cms_MembershipUser AS MemberSHIP
          ON
       MemberROLE.MembershipID = MemberSHIP.MembershipID
	  and MemberROLE.DBNAME = MemberSHIP.DBNAME
          JOIN BASE_HFit_PPTEligibility AS PPT
          ON
       PPT.UserID = MemberSHIP.UserID
	  and PPT.DBNAME = MemberSHIP.DBNAME
          JOIN BASE_CMS_USERSettings AS USERSET
          ON
       USERSET.UserSettingsUserID = PPT.UserID
	  and USERSET.DBNAME = PPT.DBNAME
          JOIN BASE_OM_ContactGroupMember AS GRPMBR
          ON
       GRPMBR.ContactGroupMemberRelatedID = USERSET.HFitPrimaryContactID
	  and GRPMBR.DBNAME = USERSET.DBNAME
          JOIN BASE_OM_ContactGroup AS GRP
          ON
       GRP.ContactGroupID = GRPMBR.ContactGroupMemberContactGroupID
	  and GRP.DBNAME = GRPMBR.DBNAME
          JOIN BASE_HFit_ContactGroupMembership AS GroupMBR
          ON
       GroupMBR.cmsMembershipID = MemberSHIP.MembershipID
	  and GroupMBR.DBNAME = MemberSHIP.DBNAME
          JOIN BASE_HFit_Account AS ACCT
          ON
       ROLES.SiteID = ACCT.SiteID
	  and ROLES.DBNAME = ACCT.DBNAME
          JOIN BASE_CMS_Site AS SITE
          ON
       SITE.SiteID = ACCT.SiteID
	  and SITE.DBNAME = ACCT.DBNAME
...
GO
PRINT 'Executed view_EDW_Eligibility_MART.sql';
GO
