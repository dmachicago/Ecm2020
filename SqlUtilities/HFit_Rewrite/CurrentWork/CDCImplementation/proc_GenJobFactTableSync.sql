...
IF EXISTS (SELECT
                  name
                  FROM sys.procedures
                  WHERE name = 'proc_GenJobFactTableSync') 
    BEGIN
        DROP PROCEDURE
             proc_GenJobFactTableSync;
    END;
GO
-- exec proc_GenJobFactTableSync 3, 'proc_FACT_CMS_Document_KenticoCMS_2_Insert'
CREATE PROCEDURE proc_GenJobFactTableSync
(
     @Interval AS int = 1
...
BEGIN
  ...
  IF CHARINDEX ('_3_', @ProcName) > 0
    ...
    @command = @CMD,
    @database_name = N'KenticoCMS_DataMart',
    @flags = 0;
    IF @@ERROR <> 0
    OR @ReturnCode <> 0
        BEGIN GOTO QuitWithRollback;
