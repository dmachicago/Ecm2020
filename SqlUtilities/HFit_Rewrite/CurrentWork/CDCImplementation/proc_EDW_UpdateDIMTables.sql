...
*/
    ...
    INTO
         #TEMP_HFit_HealthAssesmentUserRiskArea
           FROM CHANGETABLE (CHANGES FACT_HFit_HealthAssesmentUserRiskArea, 0) AS CT_HFit_HealthAssesmentUserRiskArea
           WHERE
...
/*---------------------------------------------------------------------------------------------------------------
***** Object:  Index [CI_FACT_FACT_HFIT_HEALTHASSESMENTUSERANSWERS09]    Script Date: 11/25/2015 8:38:01 AM *****
*/
    CREATE CLUSTERED INDEX CI_FACT_FACT_HFIT_HEALTHASSESMENTUSERANSWERS09 ON #TEMP_HFit_HealthAssesmentUserRiskArea
    (
    ItemID ASC ,
    SVR ASC ,
    DBNAME ASC
    ...
    DELETE D
           FROM DIM_EDW_CMS_USER D
                    JOIN CHANGETABLE (CHANGES FACT_CMS_User , @ChangeVersionID) AS CT_CMS_User
                        ON
    ...
    WITH CTE_EDW_CMS_USER (
       ...
       , USERID) 
        ...
        SELECT
             ...
             , DBNAME
             , CT_CMS_User.USERID
               FROM CHANGETABLE (CHANGES FACT_CMS_User, 0) AS CT_CMS_User
               WHERE
...
,S.LastModifiedDate = GETDATE () 
       ...
       , ItemID) 
        ...
        SELECT
             ...
             , CT_HFIT_HEALTHASSESMENTUSERSTARTED.ItemID
               FROM CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERSTARTED, @ChangeVersionID) AS CT_HFIT_HEALTHASSESMENTUSERSTARTED
               WHERE
               SYS_CHANGE_OPERATION = 'I'
        EXCEPT
        SELECT
    ...
    IF @iCnt = 0
        BEGIN
            EXEC @iCnt = proc_QuickRowCount 'FACT_HFIT_HealthAssesmentUserAnswers';
            DECLARE
...
,S.LastModifiedDate = GETDATE () 
    ...
    WITH CTE_temp (
       ...
       , ItemID) 
        ...
        SELECT
             ...
             , DBNAME
             , CTE_temp.ItemID
               FROM CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERMODULE, @ChangeVersionID) AS CTE_temp
               WHERE
               SYS_CHANGE_OPERATION = 'I'
        EXCEPT
        SELECT
    ...
    --select CT_TEMP.ItemID, CT_TEMP.SYS_CHANGE_VERSION from CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERRISKCATEGORY , NULL) AS CT_TEMP where SYS_CHANGE_OPERATION = 'U'
    ...
    DELETE D
           FROM DIM_EDW_HFIT_HEALTHASSESMENTUSERRISKCATEGORY D
                    JOIN CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERRISKCATEGORY , @ChangeVersionID) AS CT_HFIT_HEALTHASSESMENTUSERRISKCATEGORY
                        ON
                        SYS_CHANGE_OPERATION = 'D'
                    AND CT_HFIT_HEALTHASSESMENTUSERRISKCATEGORY.ItemID = D.ItemID
    ...
    DELETE D
           FROM DIM_EDW_CMS_USERSETTINGS D
                    JOIN CHANGETABLE (CHANGES FACT_CMS_USERSETTINGS , @ChangeVersionID) AS CT_CMS_USERSETTINGS
                        ON
                        SYS_CHANGE_OPERATION = 'D'
                    AND D.UserSettingsID = CT_CMS_USERSETTINGS.UserSettingsID
...
,S.LastModifiedDate = GETDATE () 
    ...
    DELETE D
           FROM DIM_EDW_HFit_HealthAssesmentUserRiskArea D
                    JOIN CHANGETABLE (CHANGES FACT_HFit_HealthAssesmentUserRiskArea , @ChangeVersionID) AS CT
                        ON
                        D.ItemID = CT.ItemID
                    AND SYS_CHANGE_OPERATION = 'D'
                    AND D.SVR = CT.SVR
                    AND D.DBNAME = CT.DBNAME;
...
,S.LastModifiedDate = GETDATE () 
    ...
    DELETE D
           FROM DIM_EDW_HFit_HealthAssesmentUserQuestionGroupResults D
                    JOIN CHANGETABLE (CHANGES FACT_HFit_HealthAssesmentUserQuestionGroupResults , @ChangeVersionID) AS CT
                        ON
                        D.ItemID = CT.ItemID
                    AND SYS_CHANGE_OPERATION = 'D'
                    AND D.SVR = CT.SVR
...
,S.LastModifiedDate = GETDATE () 
    ...
    EXEC PrintImmediate 'Populating #TEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION';
    SELECT
           CT.ITEMID
         , CT.SVR
         , CT.DBNAME
    ...
    INTO
         #TEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION
           FROM CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERQUESTION, 0) AS CT;

    RAISERROR ('Populated TEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION.' , 10 , 1) WITH NOWAIT;
    RAISERROR ('Creating IDXTEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION.' , 10 , 1) WITH NOWAIT;
    CREATE CLUSTERED INDEX IDXTEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION ON #TEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION
    (SVR, DBNAME, ITEMID, SYS_CHANGE_OPERATION, SYS_CHANGE_VERSION) ;
    RAISERROR ('Created IDXTEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION.' , 10 , 1) WITH NOWAIT;
    ...
    IF @iCnt = 0
        BEGIN
            ...
            FACT_HFIT_HEALTHASSESMENTUSERQUESTION AS USERQUES
                LEFT OUTER JOIN #TEMP_FACT_HFIT_HEALTHASSESMENTUSERQUESTION AS CT_HFIT_HEALTHASSESMENTUSERQUESTION
                    ON
    ...
    DELETE D
           FROM DIM_EDW_HFit_HealthAssesmentUserQuestion D
                    JOIN CHANGETABLE (CHANGES FACT_HFIT_HEALTHASSESMENTUSERQUESTION , @ChangeVersionID) AS CT
                        ON
                        D.ItemID = CT.ItemID
                    AND SYS_CHANGE_OPERATION = 'D'
                    AND D.SVR = CT.SVR
