GO
PRINT 'EXecuting proc_FACT_PullTime_HIST.sql';
GO
IF EXISTS ( SELECT
                   name
                   FROM sys.procedures
                   WHERE name = 'proc_FACT_PullTime_HIST') 
    BEGIN
        DROP PROCEDURE
             proc_FACT_PullTime_HIST;
    END;
GO
...
exec proc_FACT_PullTime_HIST @RowGuid, @Action, @DBMS, @TblName, @NbrRecs

select * from  dbo.FACT_PullTime_HIST
*/
CREATE PROCEDURE proc_FACT_PullTime_HIST (
...
BEGIN
    ...
    --@Action : N - add a new record to the FACT_PullTime_HIST table
    ...
    IF @Action = 'T'
        BEGIN
            ...
            @S AS datetime = ( SELECT
                                      ExecutionStartTime
                                      FROM  FACT_PullTime_HIST
                                      WHERE
                                      RowGuid = @RowGuid );
            DECLARE
            @MS AS float = DATEDIFF ( SECOND , @S , GETDATE ()) ;
            PRINT '@MS: ' + CAST (@ms AS nvarchar (50)) ;
            DECLARE
            @ET AS float = @ms / 60;
            PRINT '@ET: ' + CAST (@ET AS nvarchar (50)) ;
            UPDATE dbo.FACT_PullTime_HIST
                   SET
    ...
    IF @Action = 'N'
        BEGIN
            INSERT INTO dbo.FACT_PullTime_HIST
            (
                   RowGuid
                 , DBMS
                 , TblName
...
END;
GO
PRINT 'Executed proc_FACT_PullTime_HIST.sql';
GO