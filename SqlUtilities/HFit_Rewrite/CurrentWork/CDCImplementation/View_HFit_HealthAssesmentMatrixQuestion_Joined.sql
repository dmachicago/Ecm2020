...
go
print 'Executing View_HFit_HealthAssesmentMatrixQuestion_Joined.SQL'
go

if exists (select name from sys.views where name = 'View_HFit_HealthAssesmentMatrixQuestion_Joined')
    drop view View_HFit_HealthAssesmentMatrixQuestion_Joined;
...
CREATE VIEW dbo.View_HFit_HealthAssesmentMatrixQuestion_Joined
AS 
SELECT
          VJ.Published
        , VJ.SiteName
        ...
        , MQ.HashCode
          FROM View_CMS_Tree_Joined AS VJ
                   INNER JOIN dbo.BASE_HFit_HealthAssesmentMatrixQuestion AS MQ
                       ON VJ.DocumentForeignKeyValue = MQ.HealthAssesmentMultipleChoiceQuestionID
                      AND VJ.SVR = MQ.SVR
                      AND VJ.DBNAME = MQ.DBNAME
          WHERE ClassName = 'HFit.HealthAssesmentMatrixQuestion';
GO
print 'Executed View_HFit_HealthAssesmentMatrixQuestion_Joined.SQL'
go
