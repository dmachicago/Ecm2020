...
BEGIN
    ...
    SET @CT = GETDATE () ;
    EXEC proc_trace 'START determine if DELETED records present', @CT, NULL;
    --**************************************************************
    EXEC @iChgTypes = proc_CkHaDataChanged NULL;
    PRINT 'iChgType is: ' + CAST (@iChgTypes AS nvarchar (50)) ;
    --**************************************************************
    SET @ET = GETDATE () ;
    EXEC proc_trace 'END determine if DELETED records present', @CT, @ET;
/*-------------------------------
if @iChgTypes = 
0 - no changes
1 - updates only
...
*/
    ...
    --  drop table ##DeletedEdwHAHashkeys
    create table ##DeletedEdwHAHashkeys 
            (
                                    HAUSERSTARTED_ITEMID int  NOT NULL
                                  , USERGUID uniqueidentifier  NOT NULL
                                  , PKHASHCODE nvarchar (100) NOT NULL
    ...
    CREATE CLUSTERED INDEX CI_TEMPEdwHAHashkeys ON dbo.##DeletedEdwHAHashkeys
	   (
		    HAUSERSTARTED_ITEMID ASC,
		    USERGUID ASC,
		    PKHASHCODE ASC
	   );
    -- delete from STAGED_EDW_PkHashkey where RowNbr between 1500 and 1600
    ...
    WITH CTE_DEL (
       ...
       , PKHashCode
        ...
        INSERT INTO ##DeletedEdwHAHashkeys
        (
               HAUSERSTARTED_ITEMID
             , USERGUID
             , PKHASHCODE) 
    ...
    -- select  count(*) from ##DeletedEdwHAHashkeys ;
