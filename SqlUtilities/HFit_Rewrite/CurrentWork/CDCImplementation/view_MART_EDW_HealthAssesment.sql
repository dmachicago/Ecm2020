...
GO
PRINT 'Processing view_EDW_HealthAssesment_MART_V2';
GO
...
IF EXISTS (SELECT
           ...
           FROM sys.views
           WHERE
                  name = 'view_MART_EDW_HealthAssesment') 
    BEGIN
        DROP VIEW
             [view_MART_EDW_HealthAssesment];
    END;
GO

--Select top 100 * from view_MART_EDW_HealthAssesment

CREATE VIEW dbo.view_MART_EDW_HealthAssesment
AS
--********************************************************************************************************
...
--07.21.2016 (WDM,NM,SM) - Removed IN statement for excluded MPI's and added it as a OUTTER JOIN
--********************************************************************************************************
...
SELECT HAUserStarted.ItemID AS UserStartedItemID
  ...
  FROM dbo.BASE_HFit_HealthAssesmentUserStarted AS HAUserStarted
	  ...
	  and HAUserModule.DBNAME = HARiskCategory.DBNAME
       ...
       ON HARiskCategory.ItemID = HAUserRiskArea.HARiskCategoryItemID -- 3467083 : 18s
	  and HARiskCategory.DBNAME = HAUserRiskArea.DBNAME
       INNER JOIN
       dbo.BASE_HFit_HealthAssesmentUserQuestion AS HAUserQuestion
       ON HAUserRiskArea.ItemID = HAUserQuestion.HARiskAreaItemID -- 9,752,017 : 34s
      ...
      AND HAQuestionsView.DocumentCulture = 'en-US'
       ...
       ON HAUserRiskArea.ItemID = HAUserQuestionGroupResults.HARiskAreaItemID -- 9,752,057 : 56sec
	  and HAUserRiskArea.DBNAME = HAUserQuestionGroupResults.DBNAME
       INNER JOIN
       dbo.BASE_HFit_HealthAssesmentUserAnswers AS HAUserAnswers
       ON HAUserQuestion.ItemID = HAUserAnswers.HAQuestionItemID -- 9,968,482 : 1:40 MIN
	  and HAUserQuestion.DBNAME = HAUserAnswers.DBNAME
       LEFT JOIN
       HFit_LKP_EDW_RejectMPI rejectMPI
       ON rejectMPI.RejectMPICode = UserSettings.HFitUserMpiNumber
	  --and rejectMPI.DBNAME = UserSettings.DBNAME
  WHERE rejectMPI.RejectMPICode IS NULL; -- 9,968,482 : 1:46 Min 9968482

GO
GO

PRINT 'Processed view_EDW_HealthAssesment_MART_V2';
GO
--  
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_CMS_Document_DocumentCulture') 
    BEGIN
        CREATE NONCLUSTERED INDEX PI_CMS_Document_DocumentCulture ON dbo.BASE_CMS_Document (DocumentCulture) INCLUDE (DocumentForeignKeyValue, DocumentNodeID, SVR, DBNAME) ;
    END;

GO
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_CMS_Document_NodeClassID_SVR') 
    BEGIN
        CREATE NONCLUSTERED INDEX PI_CMS_Document_NodeClassID_SVR ON dbo.BASE_CMS_Tree (NodeClassID, SVR) INCLUDE (NodeSiteID, NodeGUID, NodeLinkedNodeID) ;
    END;

GO
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_CMS_Document_ClassName') 
    BEGIN
        CREATE NONCLUSTERED INDEX PI_CMS_Document_ClassName ON dbo.BASE_CMS_Class (ClassName) ;
    END;

GO
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'CI_CMS_Document_Culture_NodeID_SVR') 
    BEGIN
        CREATE NONCLUSTERED INDEX CI_CMS_Document_Culture_NodeID_SVR ON dbo.BASE_CMS_Document (DocumentCulture, DocumentNodeID, SVR) INCLUDE (DocumentForeignKeyValue, DBNAME) ;
    END;
--  
GO
PRINT '***** Executed: view_EDW_HealthAssesment_MART_V2.sql';
GO


IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HAUserStarted_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_CMSUser_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_UserSettings_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_UserSite_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_ACCT_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HAUserModule_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HARiskCategory_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HAUserRiskArea_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HAUserQuestion_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_HAUserAnswers_LastModifiedDate') 
    BEGIN
...
IF NOT EXISTS (SELECT name
                 FROM sys.indexes
                 WHERE name = 'PI_MART_HA_LastModifiedDate') 
    BEGIN
