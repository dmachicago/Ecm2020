...
--CreateDdlAuditTables
	...
	BEGIN
		...
		[CreateDate] [datetime2] (7)  NULL default getdate(),
		RowNbr int identity
	)
...
END
GO
--alter table SchemaChangeMonitor add RowNbr int identity (1,1)
...
GO
if exists (select name from sys.views where name = 'view_SchemaChangeMonitor_rptData ')
BEGIN
	drop view view_SchemaChangeMonitor_rptData ;
END
GO
...
BEGIN
	...
	CREATE TABLE dbo.SchemaChangeMonitor (PostTime datetime, 
		DB_User nvarchar(500), 
		IP_Address nvarchar(100), 
		CUR_User nvarchar(500), 
		OBJ nvarchar(450), 
		Event nvarchar(100), 
		TSQL nvarchar(max));
...
BEGIN
	...
	declare @CURRSVR as nvarchar(100) = '' ;
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'execute sp_SchemaMonitorReport', 
		...
		@database_name=N'KenticoCMS_ProdStaging', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
print ('Processing complete - please check for errors.' );  --  
  --  
GO 
print('***** FROM: _EDW_Fix.10.23.2014.sql'); 
GO 
