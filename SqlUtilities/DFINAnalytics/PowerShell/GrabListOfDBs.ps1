# ----------------------------------------------------------------------
# NOTE* It is always best to test any process on a single dev or 
# test server before trying it on ALL production databases!
# ----------------------------------------------------------------------
...
$databases = invoke-sqlcmd -ServerInstance Server -Database ALIEN15 -Query "SELECT INSTANCE ,DATABASENAME FROM Database.dbo.SQL_DATABASES WHERE REGION = 'PROD'"
