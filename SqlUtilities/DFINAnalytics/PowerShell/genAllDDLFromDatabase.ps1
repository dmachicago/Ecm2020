...
#********************** FOR A DB Specification ************************
$Filepath='D:\Temp\MyScriptsDirectory' # local directory to save build-scripts to
$DataSource='ALIEN15' # server name and instance
$Database='DFINAnalytics'# the database to copy from
...
#********************** FOR A DB W/O SERVER Specification ************************
$Filepath='D:\Temp\MyScriptsDirectory' # local directory to save build-scripts to
$DataSource='Alien15' # server name and instance
$Database='DFINAnalytics'# the database to copy from
# set "Option Explicit" to catch subtle errors
...
<# now we will --* USEthe canteen system of SMO to specify what we want from the script. It is best to have a list of the defaults to hand and just override the defaults where necessary, but there is a chance that a later revision of SMO could change the defaults, so beware! #>
$CreationScriptOptions = new-object ("$My.ScriptingOptions") 
$CreationScriptOptions.ExtendedProperties= $true # yes, we want these
$CreationScriptOptions.DRIAll= $true # and all the constraints 
$CreationScriptOptions.Indexes= $true # Yup, these would be nice
