SELECT TheGroup.name AS [Server Group], 
...
FROM msdb.dbo.sysmanagement_shared_server_groups_internal TheGroup
     LEFT JOIN msdb.dbo.sysmanagement_shared_registered_servers_internal TheServer ON TheGroup.server_group_id = TheServer.server_group_id
WHERE TheGroup.server_type = 0 --only the Database Engine Server Group
      AND server_name IS NOT NULL
ORDER BY [Server Group], 
         [Server name];