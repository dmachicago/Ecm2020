...
SELECT TOP 100 'CPU' as TypeRun,  *
  FROM [DFINAnalytics_CPU_BoundQry2000]
union ALL
SELECT TOP 100 'IO' as TypeRun, *
  FROM [DFINAnalytics_IO_BoundQry2000]
  where RunID = 87
...
WHILE @@FETCH_STATUS = 0
 ...
 SET @stmt = 'exec dbo.UTIL_IO_BoundQry2000 ''' + @DB + ''',' + CAST(@value AS NVARCHAR(10)) + ';';
 INSERT INTO   @CmdTable(cmd)
 VALUES   (@stmt);
 SET @stmt = 'GO';
		INSERT INTO   @CmdTable(cmd) VALUES (@stmt);
 SET @stmt = 'exec dbo.UTIL_CPU_BoundQry2000 ''' + @DB + ''',' + CAST(@value AS NVARCHAR(10)) + ';';
 INSERT INTO   @CmdTable(cmd)
 VALUES   (@stmt);
 SET @stmt = 'GO';
 INSERT INTO   @CmdTable(cmd)
