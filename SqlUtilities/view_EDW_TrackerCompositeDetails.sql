--select * from [view_EDW_TrackerCompositeDetails] where [ItemModifiedWhen] between '2013-12-31 00:00:00' and '2013-12-31 23:59:59'

alter table [HFit_TrackerBloodPressure] add IsProcessedForHa bit null 
go

alter table [HFit_TrackerBloodSugarAndGlucose] add IsProcessedForHa bit null 
go

alter table [HFit_TrackerBodyMeasurements] add IsProcessedForHa bit null 
go

alter table [HFit_TrackerCholesterol] add IsProcessedForHa bit null 
go

alter table HFit_TrackerHbA1c add IsProcessedForHa bit null 
go

alter table HFit_TrackerHeight add IsProcessedForHa bit null 
go

alter table HFit_TrackerWeight add IsProcessedForHa bit null 
go
...
SELECT 'HFit_TrackerTobaccoAttestation' as TrackerNameAggregateTable
	  ,TT.[ItemID], [EventDate],TT.[IsProfessionallyCollected],TT.[TrackerCollectionSourceID],[Notes],TT.[UserID]
	  ,CollectionSourceName_Internal,CollectionSourceName_External,'MISSING' as EventName, NULL as UOM
	  ...
	  ,null as ItemGuid
	  ,C.UserGuid, PP.MPI, PP.ClientCode 
	  ,S.SiteGUID, ACCT.AccountID, ACCT.AccountCD, T.IsAvailable, T.IsCustom, isnull(T.[UniqueName],'tobaccoattestation') as UniqueName
	  ,isnull(T.[UniqueName],'HFit_TrackerTobaccoAttestation') as ColDesc
  FROM [dbo].HFit_TrackerTobaccoAttestation TT
		inner join dbo.CMS_User C on C.UserID = TT.UserID
		inner join dbo.hfit_ppteligibility PP on TT.UserID = PP.userID
		inner join dbo.HFit_Account [ACCT] on PP.ClientCode = ACCT.AccountCD
		inner join dbo.CMS_Site S on ACCT.SiteID = S.SiteID	
		inner join dbo.HFit_TrackerCollectionSource TC on TC.TrackerCollectionSourceID = TT.TrackerCollectionSourceID
		left outer JOIN dbo.[HFIT_Tracker] T on T.AggregateTableName = 'HFit_TrackerTobaccoAttestation'		
union
SELECT 'HFit_TrackerCotinine' as TrackerNameAggregateTable
	  ...
	  ,null as ItemGuid
	  ,C.UserGuid, PP.MPI, PP.ClientCode 
	  ,S.SiteGUID, ACCT.AccountID, ACCT.AccountCD, T.IsAvailable, T.IsCustom, isnull(T.[UniqueName],'cotinine') as UniqueName
	  ,isnull(T.[UniqueName],'HFit_TrackerCotinine') as ColDesc
  FROM [dbo].HFit_TrackerCotinine TT
		inner join dbo.CMS_User C on C.UserID = TT.UserID
		inner join dbo.hfit_ppteligibility PP on TT.UserID = PP.userID
		inner join dbo.HFit_Account [ACCT] on PP.ClientCode = ACCT.AccountCD
		inner join dbo.CMS_Site S on ACCT.SiteID = S.SiteID	
		inner join dbo.HFit_TrackerCollectionSource TC on TC.TrackerCollectionSourceID = TT.TrackerCollectionSourceID
		left outer JOIN dbo.[HFIT_Tracker] T on T.AggregateTableName = 'HFit_TrackerCotinine'
GO
-- W. Dale Miller
-- DMA, Limited
