...
SET @Returndeploymentorder = 1;
SET @Includewarningforcirculardependencies = 1;
SET @Validateviews = 1;

-- Put square brackets where needed.
UPDATE #tblRequestedObjects
SET
	ObjectOwnerOrSchema = CASE
						  WHEN LEFT (ObjectOwnerOrSchema, 1) <> N'['
						   AND RIGHT (ObjectOwnerOrSchema, 1) <> N']'
  ...
  ,
	ObjectName = CASE
				 WHEN LEFT (ObjectName, 1) <> N'['
				  AND RIGHT (ObjectName, 1) <> N']'
					 THEN N'[' + ObjectName + N']'
				 WHEN LEFT (ObjectName, 1) <> N'['
				  AND RIGHT (ObjectName, 1) = N']'
					 THEN N'[' + ObjectName
...
IF EXISTS (SELECT
				  *
				  FROM #tblRequestedObjects
				  WHERE OBJECT_ID (ObjectOwnerOrSchema + '.' + ObjectName) IS NULL) 
AND (@Includealldbobjects = 0
  OR @Includealldbobjects IS NULL) 
	BEGIN
...
INSERT INTO #tblAllDBRoutinesTablesViews (
			ObjectID
		  , ObjectOwnerOrSchema
		  , ObjectName
		  , ObjectType) 
SELECT
	   OBJECT_ID ('[' + ROUTINE_SCHEMA + '].[' + ROUTINE_NAME + ']') 
	 , ROUTINE_SCHEMA
	 , ROUTINE_NAME
	 , CASE
	   WHEN ROUTINE_TYPE = N'PROCEDURE'
		   THEN N'PROCEDURE'
	   WHEN ROUTINE_TYPE = N'FUNCTION'
		   THEN N'FUNCTION'
	   END
...
SELECT
	 ...
	 , TABLE_NAME
	 , CASE
	   WHEN TABLE_TYPE = N'VIEW'
		   THEN N'VIEW'
	   WHEN TABLE_TYPE = N'BASE TABLE'
		   THEN N'TABLE'
	   END
...
IF @Validateviews = 1
	BEGIN
		WHILE @Currobjectid IS NOT NULL
			BEGIN
				SELECT
					   @Currobjectownerorschema = REPLACE (ObjectOwnerOrSchema, '''', '''''') 
	...
	BEGIN
		...
		SELECT
			 ...
			 , CASE
			   ...
			   END
			   FROM
					#tblAllDBRoutinesTablesViews AS a
						INNER JOIN #tblRequestedObjects AS z
...
CREATE TABLE #MySysdepends (
		   ...
		   ,
			 ...
			 depType char (1) COLLATE database_default
							  NOT NULL
		   , idObjectOwnerOrSchema nvarchar (128) COLLATE database_default
												  NOT NULL
		   , idObjectName nvarchar (128) COLLATE database_default
										 NOT NULL
		   , idObjectType nvarchar (128) COLLATE database_default
										 NOT NULL
		   ...
		   , depidObjectOwnerOrSchema nvarchar (128) COLLATE database_default
													 NOT NULL
		   , depidObjectName nvarchar (128) COLLATE database_default
											NOT NULL
		   , depidObjectType nvarchar (128) COLLATE database_default
											NOT NULL
...
INSERT INTO #MySysdepends (
		  ...
		  , AuxiliaryInt) 
SELECT DISTINCT
	   OBJECT_ID ('[' + a.TABLE_SCHEMA + '].[' + a.TABLE_NAME + ']') 
	   ...
	   FROM
			INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS a
				...
				INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS c
				   ...
				   AND b.UNIQUE_CONSTRAINT_NAME = c.CONSTRAINT_NAME -- Only get objects of interest
				INNER JOIN #tblObjects AS d
...
SELECT DISTINCT
	 ...
	 , -- direct dependency
	   ...
	   FROM
			#tblObjects AS a
				INNER JOIN sys.sql_dependencies AS b
					ON a.ObjectID = b.object_id
				INNER JOIN #tblAllDBRoutinesTablesViews AS c
					ON b.referenced_major_id = c.ObjectID
				LEFT OUTER JOIN #MySysdepends AS x
					ON a.ObjectID = x.id
...
SELECT DISTINCT
	 ...
	 , d.ObjectType
	 , 'C'
	 , -- dependency via child-object
	   2
	   FROM
			#tblObjects AS a
				INNER JOIN sysobjects AS b
					ON a.ObjectID = b.parent_obj
				INNER JOIN sys.sql_dependencies AS c
					ON b.id = c.object_id
				INNER JOIN #tblAllDBRoutinesTablesViews AS d
	...
	BEGIN
		...
		SELECT
			 ...
			 , N'TRIGGER'
			   FROM
					sysobjects AS a
						INNER JOIN #tblAllDBRoutinesTablesViews AS b
...
SET
	ObjectTextWithoutComments = ObjectText;
...
SELECT
	   @Currdatalength = ISNULL (DATALENGTH (ObjectText) , 0) 
	   FROM #tblTextBasedObjects
	   WHERE ObjectID = @Currobjectid;
...
WHILE @Currobjectid IS NOT NULL
	BEGIN
		...
		SELECT
			   @I = PATINDEX ('%/*%', ObjectTextWithoutComments) 
			   FROM #tblTextBasedObjects
			   WHERE ObjectID = @Currobjectid;
		...
		WHILE @I > 0
		  AND @Stoploop = 0
			BEGIN
				...
				WHILE @Incomment = 1
				  AND @Stoploop = 0
					BEGIN
						...
						SELECT
							   @Tmpstring = SUBSTRING (ObjectTextWithoutComments, @I, 2) 
							   FROM #tblTextBasedObjects
							   WHERE ObjectID = @Currobjectid;
				...
				UPDATE #tblTextBasedObjects
				SET
					ObjectTextWithoutComments = SUBSTRING (ObjectTextWithoutComments, 1, @Cnt) + SUBSTRING (ObjectTextWithoutComments, @Cnt + @Diff + 1, @Currdatalength) -- @CurrTxtPtr @Cnt @Diff @tmpString
				WHERE
		...
		WHILE @I > 0
		  AND @Stoploop = 0
			BEGIN
				...
				SELECT
					   @Cnt1 = PATINDEX ('%' + CHAR (10) + '%', SUBSTRING (ObjectTextWithoutComments, @I, @Currdatalength)) 
...
SET @Startstr = N'%[' + CHAR (9) + CHAR (10) + CHAR (13) + CHAR (32) + CHAR (33) + CHAR (34) + CHAR (37) + CHAR (38) + CHAR (40) + CHAR (41) + CHAR (42) + CHAR (43) + CHAR (44) + CHAR (45) + CHAR (46) + CHAR (47) + CHAR (58) + CHAR (59) + CHAR (60) + CHAR (61) + CHAR (62) + CHAR (94) + CHAR (124) + CHAR (126) + N']';
...
SET @Endstr = N'[' + CHAR (9) + CHAR (10) + CHAR (13) + CHAR (32) + CHAR (33) + CHAR (34) + CHAR (37) + CHAR (38) + CHAR (40) + CHAR (41) + CHAR (42) + CHAR (43) + CHAR (44) + CHAR (45) + CHAR (46) + CHAR (47) + CHAR (58) + CHAR (59) + CHAR (60) + CHAR (61) + CHAR (62) + CHAR (94) + CHAR (124) + CHAR (126) + N']%';
...
UPDATE a
SET
	a.ObjectNameForLikeSearches1 = CASE
								   WHEN a.ObjectName LIKE N'%''%'
									 OR a.ObjectName LIKE N'% %'
									   THEN N'%[[]' + a.ObjectName + ']%'
...
******/
		...
		SELECT DISTINCT
			 ...
			 , c.ObjectType
			 , 'S'
			 , -- direct dependency found in text-based searches
			   2
			   FROM
					#tblObjects AS a
						INNER JOIN #tblTextBasedObjects AS b
							-- Text of object in #tblObjects that contains a reference to other objects.
							ON a.ObjectID = b.ObjectID
						   AND b.ObjectType <> 'TRIGGER' -- The reference can be to any object in the database.
						INNER JOIN #tblAllDBRoutinesTablesViews AS c
		...
		SELECT DISTINCT
			 ...
			 , d.ObjectType
			 , 'T'
			 , -- child-object dependency found in text-based searches
			   2
			   FROM
					#tblObjects AS a
						INNER JOIN sysobjects AS b
							ON a.ObjectID = b.parent_obj -- Only consider text of children of objects in #tblObjects.
						INNER JOIN #tblTextBasedObjects AS c
							ON b.id = c.ObjectID
					 ...
					 , -- child-object dependency found in text-based searches
					   ...
					   FROM
							#MySysdepends AS a
								INNER JOIN sysobjects AS b
									ON a.AuxiliaryInt = 1
								   AND a.depid = b.parent_obj -- Only consider text of children of objects in #tblObjects.
								INNER JOIN #tblTextBasedObjects AS c
									ON b.id = c.ObjectID
								   AND c.ObjectType = 'TRIGGER'
								INNER JOIN #tblAllDBRoutinesTablesViews AS d
...
******/
			   ...
			   FROM
					#tblAllDBRoutinesTablesViews AS a
						INNER JOIN #tblTextBasedObjects AS b
							...
							ON a.ObjectID = b.ObjectID
						   AND b.ObjectType <> 'TRIGGER' -- The reference can be to objects in #tblObjects.
						INNER JOIN #tblObjects AS c
							ON a.ObjectID <> c.ObjectID -- ignore self-dependency since it will cause another loop
						LEFT OUTER JOIN #MySysdepends AS x
							ON a.ObjectID = x.id
		...
		SELECT DISTINCT
			 ...
			 , -- child-object dependency found in text-based searches
			   ...
			   FROM
					#tblAllDBRoutinesTablesViews AS a
						INNER JOIN sysobjects AS b
							ON a.ObjectID = b.parent_obj -- Only consider text of children of objects in #tblAllDBRoutinesTablesViews.
						INNER JOIN #tblTextBasedObjects AS c
							ON b.id = c.ObjectID
						   AND c.ObjectType = 'TRIGGER'
						INNER JOIN #tblObjects AS d
		...
		UPDATE a
		SET
			a.idObjectNameForLikeSearches1 = CASE
											 WHEN a.idObjectName LIKE N'%''%'
											   OR a.idObjectName LIKE N'% %'
												 THEN N'%[[]' + a.idObjectName + ']%'
		  ...
		  ,
			a.depidObjectNameForLikeSearches1 = CASE
												WHEN a.depidObjectName LIKE N'%''%'
												  OR a.depidObjectName LIKE N'% %'
													THEN N'%[[]' + a.depidObjectName + ']%'
			   ...
			   FROM
					#tblObjects AS a
						INNER JOIN #tblTextBasedObjects AS b
							-- Text of object in #tblObjects that contains a reference to other objects.
							ON a.ObjectID = b.ObjectID
						   AND b.ObjectType <> 'TRIGGER' -- The reference can be to objects in the #tblObjects.
						INNER JOIN #tblObjects AS c
							ON a.ObjectID <> c.ObjectID -- ignore self-dependency since it will cause another loop
						LEFT OUTER JOIN #MySysdepends AS x
							ON a.ObjectID = x.id
		...
		WHILE @Cnt > 0
			BEGIN
				...
				SELECT DISTINCT
					 ...
					 , a.depidObjectType
					 , CASE
					   WHEN a.depidObjectType = 'TABLE'
						   THEN 1
					   WHEN a.depidObjectType = 'VIEW'
						   THEN 2
					   WHEN a.depidObjectType = 'PROCEDURE'
						   THEN 3
					   WHEN a.depidObjectType = 'FUNCTION'
						   THEN 4
					   END
					   FROM
							#MySysdepends AS a -- Get objects on which items it #tblObjects depend
								INNER JOIN #tblObjects AS b
									ON a.id = b.ObjectID -- And that are not yet in #tblObjects
								LEFT OUTER JOIN #tblObjects AS c
		...
		WHILE @Cnt > 0
			BEGIN
				...
				SELECT DISTINCT
					 ...
					 , a.idObjectType
					 , CASE
					   WHEN a.idObjectType = 'TABLE'
						   THEN 1
					   WHEN a.idObjectType = 'VIEW'
						   THEN 2
					   WHEN a.idObjectType = 'PROCEDURE'
						   THEN 3
					   WHEN a.idObjectType = 'FUNCTION'
						   THEN 4
					   END
					   FROM
							#MySysdepends AS a -- Get objects that items in #tblObjects depend on
								INNER JOIN #tblObjects AS b
									ON a.depid = b.ObjectID -- And that are not yet in #tblObjects
								LEFT OUTER JOIN #tblObjects AS c
									ON a.id = c.ObjectID
					   WHERE c.ObjectID IS NULL;
...
IF OBJECT_ID ('tempdb..#tblDependencyAndDeploymentOrder') IS NOT NULL
	BEGIN
		DROP TABLE
			 #tblDependencyAndDeploymentOrder
	END;
...
CREATE TABLE #tblDependencyAndDeploymentOrder (
		   ...
		   , ObjectName nvarchar (128) COLLATE database_default
									   NOT NULL
		   , QualifiedNameForObjectIDFn nvarchar (264) COLLATE database_default
													   NOT NULL
		   , ObjectType nvarchar (128) COLLATE database_default
									   NOT NULL
			...
			#tblObjects AS a
				LEFT OUTER JOIN (
								...
								SELECT DISTINCT
									   ...
									   FROM
											#tblObjects AS z
												INNER JOIN #MySysdepends AS b
													ON b.id = z.ObjectID
												INNER JOIN #tblObjects AS c
													ON b.depid = c.ObjectID
									   WHERE b.id <> b.depid) AS RS1
					ON a.ObjectID = RS1.ObjectID
	   WHERE RS1.ObjectID IS NULL
	   ORDER BY
				a.AuxiliaryOrdering, a.ObjectOwnerOrSchema, a.ObjectName;
...
WHILE @Cnt > 0
	BEGIN
		...
		SELECT
			 ...
			 , @I
			   FROM
					#tblObjects AS a -- Objects in #tblObjects and not in #tblDependencyAndDeploymentOrder
									...
									SELECT DISTINCT
										   ...
										   FROM
												#tblObjects AS b
													INNER JOIN #MySysdepends AS c
														ON c.id = b.ObjectID
													INNER JOIN #tblDependencyAndDeploymentOrder AS d
														ON d.ObjectID = c.depid
													LEFT OUTER JOIN #tblDependencyAndDeploymentOrder AS e WITH (NOLOCK) 
														ON c.id = e.ObjectID
										   WHERE e.ObjectOwnerOrSchema IS NULL) AS RS1
							ON RS1.id = a.ObjectID -- Objects in #tblObjects that depend on objects not in #tblDependencyAndDeploymentOrder
						LEFT OUTER JOIN (
										 SELECT DISTINCT
												w.id
												FROM
													 #tblObjects AS v
														 INNER JOIN #MySysdepends AS w
															 ON w.id = v.ObjectID
														 INNER JOIN #tblObjects AS x
															 ON w.depid = x.ObjectID
														 LEFT OUTER JOIN #tblDependencyAndDeploymentOrder AS y
															 ON y.ObjectID = w.depid
												WHERE w.id <> w.depid
												  AND y.ObjectOwnerOrSchema IS NULL) AS RS2
	...
	BEGIN
		...
		SELECT
			 ...
			 , 'Child objects (e.g., triggers, etc.) depende on objects that cause a circular dependency'
			   FROM
					#tblObjects AS a
						LEFT OUTER JOIN #tblDependencyAndDeploymentOrder AS b
							ON a.ObjectID = b.ObjectID
			 ...
			 , 'Circular dependency was found for the object'
			   FROM
					#tblObjects AS a
						LEFT OUTER JOIN #tblDependencyAndDeploymentOrder AS b
							ON a.ObjectID = b.ObjectID
						LEFT OUTER JOIN #tblCircularDependencies AS c
...
IF @Returnfounddependencies = 1
	BEGIN
		SELECT
			   'The table below contains all dependencies found';
		...
		SELECT
			 ...
			 , CASE
			   WHEN depType = 'D'
				   THEN 'Direct dependency found in sys.sql_dependencies'
			   WHEN depType = 'C'
				   THEN 'Dependency via child-objects found in sys.sql_dependencies'
			   WHEN depType = 'S'
				   THEN 'Direct dependency found in text-based searches'
			   WHEN depType = 'T'
				   THEN 'Dependency via child-objects found in text-based searches'
			   END AS depType
			   FROM #MySysdepends;
	END;
...
IF @Returndeploymentorder = 1
	BEGIN
		SELECT
			   'The table below contains the object deployment order and dependency hierarchy level';
...
IF @Includewarningforcirculardependencies = 1
	BEGIN
		SELECT
			   'The table below contains the circular dependencies found';
