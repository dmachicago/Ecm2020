...
/****** Object:  View [dbo].[GLOBAL_BI]    Script Date: 09/22/2010 14:54:16 ******/
SET ANSI_NULLS ON
GO
...
JOIN SMART_GLOBAL_DW.dbo.DM_D_CUSTOMER_TYPE CT  ON C.CUST_TYPE_KEY=CT.CUST_TYPE_KEY
JOIN SMART_GLOBAL_DW.dbo.DM_D_CALENDAR T ON T.TIME_KEY=S.TIME_KEY

LEFT OUTER JOIN SMART_GLOBAL_DW.dbo.DM_D_ITEM_CLASS IC ON IC.ITEM_CLASS_KEY = I.ITEM_CLASS_KEY

/*
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BPCS_IIM IIM ON UPPER(I.ITEM_NBR) = UPPER(IIM.IPROD) AND I.SRC_SYS_ID=IIM.SRC_SYS_ID AND IIM.CURRENT_IND='Y'
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BPCS_INCX INCX ON IIM.ICLAS = INCX.CXCLAS AND I.SRC_SYS_ID=INCX.SRC_SYS_ID AND INCX.CURRENT_IND='Y'
LEFT OUTER JOIN SMART.dbo.TPR_BA_ITEMRECAPCLASS RECAP ON INCX.CXREF3 = RECAP.SalesIncentiveRecapClassID
LEFT OUTER JOIN SMART.dbo.TPR_X_RECAPCLASS_PRODUCTGROUP PG ON RECAP.SalesIncentiveRecapClassID=PG.SalesIncentiveRecapClassID

*/
-- Removed hard coded h2.Hierarchy_Id=16 and replaced it with a look up.  RLL 9/22/2008
LEFT OUTER JOIN (
	SELECT H2.SRC_SYS_ID, H2.ITEM_NBR, H2.LEVEL01_NAME, H2.LEVEL02_NAME, H2.LEVEL02_CD 
		FROM SMART_GLOBAL_DW.dbo.DM_H_ITEM h2
		INNER JOIN SMART_GLOBAL_DW.dbo.DM_L_HIERARCHY l1 ON h2.Hierarchy_Id=l1.Hierarchy_Id
...
LEFT OUTER JOIN (SELECT H4.SRC_SYS_ID, H4.ITEM_NBR, H4.PRODUCT,H4.PERFORMANCE, H4.MATERIAL, H4.PRIMARY_MKT, H4.ENVIROMENT, H4.BRAND, H4.SPC_STD_FLG, H4.NORM_FACTOR FROM SMART_GLOBAL_DW.dbo.DM_D_ITEM H4
...
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.DM_D_CUSTOMER_TYPE CT  ON C.CUST_TYPE_KEY=CT.CUST_TYPE_KEY
...
LEFT OUTER JOIN (
	SELECT H2.SRC_SYS_ID, H2.ITEM_NBR, H2.LEVEL01_NAME, H2.LEVEL02_NAME, H2.LEVEL02_CD 
		FROM BDCSQLSP.SMART_DB.dbo.DM_H_ITEM h2
		INNER JOIN BDCSQLSP.SMART_DB.dbo.DM_L_HIERARCHY l1 ON h2.Hierarchy_Id=l1.Hierarchy_Id
		WHERE H2.Current_Ind = 'Y' AND H2.Delete_Ind = 'N' AND l1.NAME='ITEM_GL'
) h ON h.ITEM_NBR = I.ITEM_NBR AND h.SRC_SYS_ID = S.SRC_SYS_ID
...
SELECT  --(I.ITEM_KEY),
    	...
    	B.SellingUnitOfMeasure Item_Sell_UM_Conversion,
...
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.DM_D_CALENDAR T ON T.TIME_KEY=S.TIME_KEY
/*
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.DM_D_ITEM_CLASS IC ON IC.ITEM_CLASS = B.ItemClass
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.CON_D_ITEMCLASS IC ON IC.ItemClass = B.ItemClass AND IC.SRC_SYS_ID = S.SRC_SYS_ID
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BPCS_IIM IIM ON UPPER(I.ITEM_NBR) = UPPER(IIM.IPROD) AND I.SRC_SYS_ID=IIM.SRC_SYS_ID AND IIM.CURRENT_IND='Y'
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.DM_F_INVOICE_HDR HDR ON HDR.INVOICE_NBR  = S.INVOICE_NBR AND HDR.SRC_SYS_ID = S.SRC_SYS_ID AND HDR.TIME_KEY = S.TIME_KEY
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BA_ITEMRECAPCLASS RECAP ON INCX.CXREF3 = RECAP.SalesIncentiveRecapClassID
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BPCS_INCX INCX ON IC.ITEM_CLASS = INCX.CXCLAS AND S.SRC_SYS_ID=INCX.SRC_SYS_ID AND INCX.CURRENT_IND='Y'
LEFT OUTER JOIN TPR_BA_ITEMRECAPCLASS RECAP ON INCX.CXREF3 = RECAP.SalesIncentiveRecapClassID
LEFT OUTER JOIN BDCSQLSP.SMART_DB.dbo.TPR_BPCS_INCX INCX ON B.ItemClass = INCX.CXCLAS AND S.SRC_SYS_ID=INCX.SRC_SYS_ID AND INCX.CURRENT_IND='Y'
*/
...
LEFT OUTER JOIN (SELECT CASE WHEN SRC_SYS_ID= 'TEXTAL' THEN 'BPCSAL'
                             ...
                             WHEN SRC_SYS_ID= 'TEXTEU' THEN 'BPCSEU'
                             WHEN SRC_SYS_ID= 'TEXTMH' THEN 'BPCSMH'
                             WHEN SRC_SYS_ID= 'dbo.ESS_D_ORGANIZATION_VWTEXTRI' THEN 'BPCSRI'
                             WHEN SRC_SYS_ID= 'TEXTTM' THEN 'THERMX'
                             WHEN SRC_SYS_ID= 'TEXTWP' THEN 'STDMWP' END AS SRC_SYS_ID,
							 ACCT_NBR AS CUSTOMER_NBR,
							 GBL_NAME AS LEVEL01_NAME,
...
LEFT OUTER JOIN (SELECT H4.SRC_SYS_ID, H4.ITEM_NBR, H4.PRODUCT,H4.PERFORMANCE, H4.MATERIAL, H4.PRIMARY_MKT, H4.ENVIROMENT, H4.BRAND, H4.SPC_STD_FLG, H4.NORM_FACTOR FROM BDCSQLSP.SMART_DB.dbo.DM_D_ITEM H4
 WHERE H4.Current_Ind = 'Y') ih ON ih.ITEM_NBR = I.ITEM_NBR AND ih.SRC_SYS_ID = S.SRC_SYS_ID
