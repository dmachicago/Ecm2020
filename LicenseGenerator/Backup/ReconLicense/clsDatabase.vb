...
Public Class clsDatabase
    ...
    Public Sub setConnStr()
        ...
        Dim S$ = ""
        DBDIR = DQ + DBDIR + DQ
        DBDIR = DQ + "C:\Program Files\Microsoft SQL Server\MSSQL.1\MSSQL\Data\org_db.mdf" + DQ
        'S$ = "Data Source=.\SQLEXPRESS;AttachDbFilename=" + DBDIR + ";Integrated Security=True;Connect Timeout=30;User Instance=True "
        'S = "Data Source=LITTLEGUY\SQLEXPRESS;Integrated Security=True"
        'S = "Data Source=.\SQLEXPRESS;AttachDbFilename=C:\DEV\DCO\Admin\App_Data\ASPNETDB.MDF;Integrated Security=True;Connect Timeout=30;User Instance=True"
        'S = "Data Source=.\SQLEXPRESS;AttachDbFilename=" + DBDIR + ";Integrated Security=True;Connect Timeout=30;User Instance=True"
        'S = "Data Source=.\SQLExpress;Integrated Security=True;User Instance=True;AttachDBFilename=|DataDirectory|Personal.mdf "
        'S = "Data Source=LITTLEGUY\sqlexpress;Initial Catalog=DMA.UD;Integrated Security=True"        
        'Dim oConn As New SqlConnection(ConfigurationSettings.AppSettings("DB_CONN_STR"))
        S = System.Configuration.ConfigurationManager.AppSettings("DMA_UD_License").tostring
        'S = System.Configuration.ConfigurationSettings.AppSettings("DMA_UD_License")
    ...
    End Function
    Public Function iCount(ByVal tSql$) As Integer
        Try
            Dim S$ = tSql
            CkConn()
            Dim Cnt As Integer
    ...
    End Function
    Public Function getCountDataSourceFiles(ByVal UserID$, ByVal FQN$, ByVal VerNO As Integer) As Integer
        FQN$ = DMA.RemoveSingleQuotes(FQN$)
    ...
    Public Function UpdateSourcefile(ByVal SourceGuid As String, ByVal LastAccessDate$, ByVal CreateDate$, ByVal LastWriteTime$, ByVal VersionNbr As Integer, ByVal UploadFQN$) As Boolean
        ...
        Return B
    End Function
    Public Function updateSourcefile(ByVal SourceGuid As String, _
                   ByVal UploadFQN$, _
        ...
        Return B
    End Function
    Public Function isSourcefileOlderThanLastEntry(ByVal UserID$, ByVal SourceGuid As String, _
                   ByVal UploadFQN$, _
                   ByVal SourceName$, _
                   ByVal SourceTypeCode$, _
                   ByVal FileLength$, _
    ...
    Public Function ExecuteSql(ByVal sql As String, ByVal ConnStrID$) As Boolean
        ...
        Dim TxName As String = "TX001"
        Dim rc As Boolean = False
        Dim ConnStr$ = System.Configuration.ConfigurationManager.AppSettings("DMA_UD_License")
    ...
    '        If rsColInfo.HasRows Then
    '            Do While rsColInfo.Read()
    '                EmailGuid = rsColInfo.GetValue(0).ToString
    '                CC = rsColInfo.GetValue(1).ToString
    ...
    '                            Dim SS$ = "UPDATE [DMA.UD].[dbo].[Recipients]"
    ...
    '            Dim bb As Boolean = ExecuteSql(S)
    '            If Not bb Then
    '                Debug.Print("ERROR: " + S)
    '            End If
    '        Next
    ...
    '        Dim RECIPS As New clsRECIPIENTS
    '        Dim SL As New SortedList
    '        Dim S$ = " SELECT [EmailGuid] ,[Recipient] ,[TypeRecp] FROM [DMA.UD].[dbo].[Recipients] order by EmailGuid "
    ...
    '                II += 1
    '                EmailGuid = rsColInfo.GetValue(0).ToString
    '                Recipient$ = rsColInfo.GetValue(1).ToString
    '                TypeRecp$ = rsColInfo.GetValue(2).ToString
    '                If II = 1 Then
    '                    PrevGuid$ = EmailGuid
    '                End If
    '                If Not EmailGuid.Equals(PrevGuid$) Then
    '                    Dim SS$ = "UPDATE [DMA.UD].[dbo].[Email]"
    ...
    '                FQN = rsColInfo.GetValue(1).ToString
    ...
    '        Dim RECIPS As New clsRECIPIENTS
    '        Dim SL As New SortedList
    '        Dim S$ = " SELECT [EmailGuid] ,[Recipient] ,[TypeRecp] FROM [DMA.UD].[dbo].[Recipients] where TypeRecp = 'CC' order by EmailGuid "
    ...
    '    End Sub
    '    Sub getExcludedEmails(ByVal UserID$)
    '        Dim L As New List(Of String)
    '        Dim RECIPS As New clsRECIPIENTS
    '        Dim SL As New SortedList
    '        Dim S$ = " SELECT [FromEmailAddr] FROM [DMA.UD].[dbo].[ExcludeFrom] where  Userid = '" + UserID + "' "
    ...
    '        If rsColInfo.HasRows Then
    '            Do While rsColInfo.Read()
    '                Email = rsColInfo.GetValue(0).ToString
    ...
    '        rsColInfo = Nothing
    '    End Sub
    '    Public Function getDirectoryParms(ByRef A$(), ByVal FQN As String, ByVal UserID As String) As Boolean
    '        FQN = DMA.RemoveSingleQuotes(FQN)
    '        Dim B As Boolean = False
    '        ReDim A(0)
    '        Dim IncludeSubDirs$ = ""
    ...
    End Function
    Sub xPopulateComboBox(ByRef CB As ComboBox, ByVal TblColName$, ByVal S$)
        Try
            CkConn()
            If gConn.State = ConnectionState.Closed Then
                gConn.Open()
    ...
    End Function
    Sub PopulateComboBox(ByRef CB As ComboBox, ByVal TblColName$, ByVal S$)
        ...
        End Try
    End Sub
    Sub PopulateListBox(ByRef LB As ListBox, ByVal TblColName$, ByVal S$)
        Try
    ...
    Function LoadLicenseFile(ByVal FQN$) As String
        ...
        Return ""
    End Function
    Function UploadLicense(ByVal CompanyID$, _
                           ByVal LicenseID$, _
                           ByVal MachineID$, _
                           ByVal EncryptedLicense$) As Boolean
        ...
        Dim licenseConnStr$ = System.Configuration.ConfigurationManager.AppSettings("LicenseServer").ToString
