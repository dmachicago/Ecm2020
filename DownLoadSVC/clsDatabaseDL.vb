...
Public Class clsDatabase
    ...
    Sub getFileParameters(ByVal EncCS As String, ByVal TgtGuid As String, ByRef FileName As String, ByRef fExt As String, ByVal TgtTable As String, ByRef RC As Boolean)
        ...
        If EncCS.Length > 0 Then
            EncCS = ENC.AES256DecryptString(EncCS)
        End If
        ...
        Try
            ...
            Dim CN As New SqlConnection(EncCS)

            Dim RSData As SqlDataReader = Nothing
            'RSData = SqlQryNo'Session(S)

            Dim CONN As New SqlConnection(EncCS)
            CONN.Open()
    ...
    Sub getPreviewFileSourceGuid(ByVal EncCS As String, ByVal UserID As String, ByRef RetGuids As Dictionary(Of String, String), ByRef RC As Boolean)
        ...
        Finally
            If AllGuids.Trim.Length > 0 Then
                ...
                Dim B As Boolean = ExecuteSqlNewConn(995611, UserID, tSql, RetMSg)
            End If
        End Try
    ...
    Sub getRestoreFileSourceGuid(ByVal EncCS As String, ByVal UserID As String, ByRef RetGuids As Dictionary(Of String, String), ByRef RC As Boolean)
        ...
        Try
            ...
            Dim RSData As SqlDataReader = Nothing
            'RSData = SqlQryNo'Session(S)
            Dim CS As String = getConnStr() : Dim CONN As New SqlConnection(CS) : CONN.Open() : Dim command As New SqlCommand(S, CONN) : RSData = command.ExecuteReader()
            If RSData.HasRows Then
                Do While RSData.Read()
                    ContentGuid = RSData.GetValue(0).ToString
                    ContentType = RSData.GetValue(1).ToString
        ...
        Finally
            If AllGuids.Trim.Length > 0 Then
                ...
                AllGuids = AllGuids.Substring(0, AllGuids.Length - 1) + ")"
                Dim tSql As String = "Update [RestoreQueue] set StartDownloadTime = getdate() where RowGuid in " + AllGuids
                Dim B As Boolean = ExecuteSqlNewConn(995612, UserID, tSql, RetMSg)
            End If
        End Try
    ...
    Public Sub setConnStr()
        ...
        setConnectionStringTimeout(S, TimeOutSecs)
        gConnStr = S
    End Sub
    ...
    Public Function ExecuteSqlNewConn(ByVal LocationID As Integer, ByVal UserGuidID As String, ByVal MySql As String, ByRef RetMsg As String) As Boolean
        RetMsg = ""
        ...
        Try
            Using CN
                ...
                Catch ex As Exception
                    rc = False
                    RetMsg = "Location LocationID: " & LocationID & " UID: " & UserGuidID & " : " & ex.Message
                End Try
            End Using
        Catch ex As Exception
            BB = False
